...
label_to_index = {
    label: i for i, label in enumerate(set(string_labels))
}
...
class SimpleClassifier(nn.Module):
    def __init__(self, input_dim, hidden_dim,hidden_dim2,hidden_dim3,hidden_dim4, output_dim):
       ...
       self.relu = nn.ReLU()
       self.fc3 = nn.Linear(hidden_dim, hidden_dim3)
       self.relu = nn.ReLU()
...
def classify_text(text, model, char_to_index, vocab_size, max_len, index_to_label):
    ...
    print(f"转换为概率: {probabilities.shape}")
    # print(f"概率总和: {probabilities.sum().item()}")
